...
public class SystemUserScreen extends Screen {
    ...
    public SystemUserScreen(WindowInt wind) throws Exception {
        ...
        if (userPermission == null)
            throw new PermissionException(Messages.get().screenPerm("System User Screen"));
            ...
            public void onDrop(DropEvent<DragItem> event) {
                ...
                if (item.getSource() == appModuleTable) {
                    ...
                    if (moduleAddedtoUser(mod.getId())) {
                        Window.alert(Messages.get().moduleAddedToUser());
                        return;
                    }
            ...
            public void onDrop(DropEvent<DragItem> event) {
                ...
                if (item.getSource() == appSectionTable) {
                    ...
                    if (sectionAddedtoUser(sec.getId())) {
                        Window.alert(Messages.get().sectionAddedToUser());
                        return;
                    }
    ...
    protected void delete(ClickEvent event) {
        boolean ok;
        ok = Window.confirm(Messages.get().deleteUserMessage());
        if ( !ok)
            return;
        ...
        try {
            ...
            if (manager.module.count() == 0 && manager.section.count() == 0 &&
                ...
                abort(event);
                DataChangeEvent.fire(this);
                setError(Messages.get().userDeleteInvalid());
                removeBusy();
                return;
            }
    ...
    protected void duplicate() {
        try {
            ...
            if (manager.module.count() == 0 && manager.section.count() == 0) {
                DataChangeEvent.fire(this);
                setError(Messages.get().userDuplicateInvalid());
                removeBusy();
                return;
            }
            clearKeys();
